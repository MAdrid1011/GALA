...
1. 运行六项 NCU v3 invocation campaign 并补齐 AGX Orin 校准向量；完整 CUDA Event、九窗口 NSYS exact-kernel capture 和基于真实 inventory 的计划已通过结构门。v3 plan 位于 `GALA-runtime/records/r2_gaussian_chest_ncu_signature_plan_v3.json`（仓库外），内容 hash 为 `8a05c51ca37ce333a4b149b3e3a6e8d6471b31b51d35263b35ac1413945da718`，绑定 campaign `75d3968373271d8b019f20279ab0aaf47d145fd1d337b02c16f0ce8384ae593d`，覆盖 `174421` 个 NSYS kernel、`1173` 个全局签名和 `2400` 个代表迭代签名。计划把 `4515` 个 first/middle/last 必需锚点合并为 6 次完整训练，并显式列出合并产生的 `4514` 个额外真实 launch，共要求逐一测量和验收 `9029` 个 launch。NCU v3 job-1 短门已通过：输出位于 `GALA-runtime/records/r2_gaussian_chest_ncu_v3_preflight_job1c/`，16 个实际 launch、SourceCounters report SHA-256 `46738c049b9738142d9729e4092fe0b0b1f7c11de8d9424751462556dd5b0c2c`，平均 GPU 利用率 `34.125%`，短门耗时 `19.2425 s`，按 native 预检和实测 profiler 开销预测正式 job-1 为 `1838.99 s`。旧 iteration 499 NVTX-call 作业自然结束且 175 个已导出 launch 的动态 SASS 分类通过，但 backward call `1084` 从 NSYS 的 128 个降为 NCU 的 1 个，因此旧证据仍为 `provisional_ncu_evidence`。仓库外线程探针已证明根因是 NVTX push/pop 栈不传播到 autograd 工作线程；`--profile-from-start off`、九个 `cudaProfilerStart/Stop` 窗口、跨线程 `gala_ncu_stage` start/end 范围和全局 invocation filter 的组合可完整捕获并恢复 stage。v3 parser 进一步绑定 NCU/stage PID、campaign/plan/job 身份及每个 launch 的 name/grid/block/stage/call/ordinal。所有作业必须让官方 30,000-iteration 训练自然结束，不得使用 `--launch-count`、`--kill` 或提前终止冒充正式证据。
