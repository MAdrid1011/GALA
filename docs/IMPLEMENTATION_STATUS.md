...
`816` 条关系、`256` 个查询和 `5,673` 个逻辑事件；在 `1,024` 个软件前沿槽下完成
`5,673/5,673` 事件，周期 `891`，峰值前沿 `1,021`，四个关系窗口全部释放。Voxel tile 0
包含 `212` 个候选、`80,063` 条关系、`512` 个查询和 `482,126` 个逻辑事件；在 `131,072`
个软件前沿槽下完成 `482,126/482,126` 事件，周期 `26,623`，峰值前沿 `131,030`，八个
关系窗口全部释放，关系记录峰值 `5,304`。两次结果均为真实 packet 的在线回放，不是合成关系
