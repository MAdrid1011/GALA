...
1. 补齐九窗口 NCU 精确 invocation campaign 和 AGX Orin 校准向量；完整 CUDA Event、九窗口 NSYS exact-kernel capture 和基于真实 inventory 的 NCU launch-signature plan 已通过结构门。当前 plan 为 `GALA-runtime/records/r2_gaussian_chest_ncu_signature_plan_v2.json`（仓库外），绑定 campaign `75d3968373271d8b019f20279ab0aaf47d145fd1d337b02c16f0ce8384ae593d`，覆盖 `174421` 个 NSYS kernel，重建 `1173` 个全局签名、`2400` 个代表迭代签名，并选择 `4515` 个 first/middle/last 校验样本。iteration 499 的完整 30,000-iteration SourceCounters 作业已自然结束，官方最终评估为 PSNR3D `35.178`、SSIM3D `0.935`、PSNR2D `49.800`、SSIM2D `0.985`；175 个实际导出的 launch 均完成动态 SASS 分类，观测阶段的 XU/atomic opcode 完整性通过。该结果同时暴露旧 plan 按 NVTX call 扩张采样的问题：iteration 499 的 13 个 selected call 在 NSYS 中有 302 个 kernel，NCU 仅导出 175 个，其中 backward call `1084` 从 128 个降为 1 个，因此证据仍为 `provisional_ncu_evidence`，不能用于正式权重。仓库外线程探针已复现并定位根因：NVTX push/pop 栈不传播到 autograd 工作线程；同一 workload 使用 `--profile-from-start off` 与 `cudaProfilerStart/Stop` 可捕获 3/3 个工作线程 kernel，叠加 `--kernel-id` 可按跨九窗口连续、窗口外不计数的 invocation ordinal 精确选择，且 NVTX start/end range 可跨线程恢复 stage 身份。正式 runner 与 parser 已据此要求 NCU Profiler API 并增加 `gala_ncu_stage` start/end 标签；下一步生成按 kernel-name 与全局 invocation 分组的新计划。所有 NCU 作业仍执行完整官方训练，不得使用 `--launch-count`、`--kill` 或提前终止冒充正式证据。
