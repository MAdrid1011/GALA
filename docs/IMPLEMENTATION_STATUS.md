...
1. 运行已冻结的完整 CUDA Event 与九窗口 NSYS/NCU campaign，并补齐 AGX Orin 校准向量；完整 CUDA Event、九窗口 NSYS exact-kernel capture 和基于真实 inventory 的 NCU launch-signature plan 已通过结构门，但正式 NCU 计数、动态 SASS 完整分类和步骤 2 仍未完整闭环。当前 plan 为 `GALA-runtime/records/r2_gaussian_chest_ncu_signature_plan_v2.json`（仓库外），绑定 campaign `75d3968373271d8b019f20279ab0aaf47d145fd1d337b02c16f0ce8384ae593d`，覆盖 `174421` 个 NSYS kernel，重建 `1173` 个全局签名、`2400` 个代表迭代签名，选择 `4515` 个 first/middle/last 校验样本，并显式采集 Nsight Compute `SourceCounters` section。所有 NCU 作业必须继续执行完整官方训练，不得使用 `--launch-count` 或 `--kill` 提前终止。当前 `499` 代表迭代的 SourceCounters campaign 正在运行；完成后才检查 source CSV 的动态指令计数并生成新的 artifact/evidence，不能沿用旧的 `provisional_ncu_evidence` 作为正式结果。
