...
- 真实 CUDA trace 旁路已接入官方 `R²-Gaussian` rasterizer/voxelizer：只在官方查询边界处于 grad-enabled 的训练路径捕获，排除 `no_grad` 质量评估、保存和报告调用；捕获完成后绑定配置哈希、模型提交和数据清单哈希，才交给周期 sink；Chunked sink 传输和依赖偏移重建已通过单元测试。
