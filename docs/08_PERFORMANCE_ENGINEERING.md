...
默认入口始终处理完整事件集合。只有显式启用开发诊断停止时，稳定窗口才可中止当前重放；该输出固定为 `development_throughput_projection`、`formal_performance_eligible=false`，不得写入正式 `cycles.json`、消融矩阵或论文表格。完整运行选项始终保留。投影最多可与非正式 AGX Orin 规格代理作数量级对照，但必须使用投影 ASIC 秒数并保留代理区间；该对照不能作为加速比门槛，也不能把事件前缀周期当作正式加速比。
